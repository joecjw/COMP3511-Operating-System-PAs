...
void algorithm_FIFO() {
    ...
    //set a counter to keep track of total page fault occurs
    int total_pf = 0;

    //initialize a fifo_queue to keep track of the item need to be replaced if page replacement is needed
    struct Queue fifo_queue;
    queue_init(&fifo_queue);
    
    //iterate through the reference string
    for(int i = 0 ; i < reference_string_length; i++){
        //iterate through the frames to check if current string is present in the frames or not
        int present = 0;
        for(int j = 0; j < frames_available; j++){
            if(reference_string[i] == frames[j]){
                present = 1;
                break;
            }
        }

        if(present == 0){//page fault ouccurs
            total_pf++;
            //append current page to the fifo_queue
            queue_enqueue(&fifo_queue,  reference_string[i]);

            if(frames[frames_available - 1] != UNFILLED_FRAME){//need page replacement
                int victim;
                int frame_value = queue_peek(&fifo_queue);
                queue_dequeue(&fifo_queue);
                for(victim = 0; victim < frames_available; victim++){
                    if(frames[victim] == frame_value){
                        break;
                    }
                }
            ...
            else{//do not need page replacement
                for(int j = 0; j < frames_available; j++){
                    if(frames[j] == UNFILLED_FRAME){
...
void algorithm_OPT() {
    ...
    for(int i = 0 ; i < reference_string_length; i++){
        ...
        if(present == 0){//page fault ouccurs
            total_pf++;
            if(frames[frames_available - 1] != UNFILLED_FRAME){//need page replacement
                if(i == reference_string_length - 1){//deal with last page(no future for reference)
                    int victim = 0;
                    for(int j = 1; j < frames_available; j++){
                        if(frames[victim] > frames[j]){
                            victim = j;
                        }
                    }
                    frames[victim] = reference_string[i];
                    ...
                    //find pages in the frames that are not used in the future
                    int fartherest, allUsed = 1;
                    int nearest[MAX_FRAMES_AVAILABLE] = {-1}; 
                    int unused[MAX_FRAMES_AVAILABLE] = {0};
                    for (int k = 0; k < frames_available; k++){
                        int z;
                        for (z = i + 1; z < reference_string_length; z++){
                            if (frames[k] == reference_string[z]){//page in frame k is used in the past at position z
                                nearest[k] = z;
                                break;
                            }
                        }

                        if (z == reference_string_length){//page in frame k is never used in the future
                            unused[k] = 1;
                            allUsed = 0;
                        }
                    }
                    
                    if(allUsed == 0){//choose from the unused pages in the future, replace frame with smaller page number
                        int victim;
                        int smallest;
                        for(int z = 0; z < frames_available; z++){
                            if(unused[z] == 1){
                                victim = z;
                                smallest = frames[victim];
                                break;
                            }
                        }

                        for(int z = victim; z < frames_available; z++){
                            if(unused[z] == 1 && frames[z] < frames[victim]){
                                victim = z;
                                smallest = frames[victim];
                            }
                        }
                        ...
                        //all of the pages in frames are used in the future, choose the fartherest one                
                        int victim = 0;
                        fartherest = nearest[0];
                        for(int z = 1; z < frames_available; z++){
                            if(nearest[z] > fartherest){
                                fartherest = nearest[z];
                                victim = z;
                            }
                        }
                        frames[victim] = reference_string[i]; 
...
void algorithm_LRU() {
    ...
    //set a array to store time when a reference string is used
    int time[MAX_FRAMES_AVAILABLE] = {0};

    //iterate through the reference string
    for(int i = 0 ; i < reference_string_length; i++){
        //set the time of current page
        ...
        if(present == 0){//page fault ouccurs
            ...
            if(frames[frames_available - 1] != UNFILLED_FRAME){//need page replacement
                int victim = 0;
                int lru = time[frames[0]];
                for (int k = 1; k < frames_available; k++){
                    if(time[frames[k]] < lru){
                        lru = time[frames[k]];
                        victim = k;
                    }
                }
