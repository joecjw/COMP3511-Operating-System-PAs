...
void algorithm_FIFO() {
    ...
    //set a counter to keep track of positon of the item need to be replaced if page replacement is needed
    int replace_index = 0;
    ...
    for(int i = 0 ; i < reference_string_length; i++){
        ...
        if(present == 0){//page fault ouccurs
            ...
            if(frames[frames_available - 1] != UNFILLED_FRAME){//need page replacement
                ...
                replace_index = (replace_index + 1) % frames_available;
            ...
            if(frames[frames_available - 1] != UNFILLED_FRAME){//need page replacement
                if(i == reference_string_length - 1){
                    ...
                    for (int k = 0; k < frames_available; k++){
                        ...
                        if (z == reference_string_length){//page in frame k is never used in the future, replace it
                    ...
                    if(isreplaced == 0){
                        //all of the pages in frames are used in the past, replace frame[victim]                
                        int victim = 0;
